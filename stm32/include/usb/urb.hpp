...
unsigned int URB::thisFrameTime() const {
	unsigned int mult = endpoint->device.speed == USBSpeed::Full?4:32;
	unsigned int time_required = 0;
	unsigned int len = buffer_len;
	if (endpoint->type == USBEndpoint::Control) {
		//apart from the data phase, Control transfers also have SETUP and STATUS.
		time_required = setupBits() * mult;
	} else if (endpoint->type == USBEndpoint::Bulk) {
...
unsigned int URB::allPacketTime() const {
	unsigned int mult = endpoint->device.speed == USBSpeed::Full?4:32;
	unsigned int time_required = 0;
...
unsigned int URB::setupTime() const {
	unsigned int mult = endpoint->device.speed == USBSpeed::Full?4:32;
	return setupBits()*mult;
}
unsigned int URB::statusTime() const {
	unsigned int mult = endpoint->device.speed == USBSpeed::Full?4:32;
	return statusBits()*mult;
}

unsigned int URB::dataTime(unsigned int length) const {
	unsigned int mult = endpoint->device.speed == USBSpeed::Full?4:32;
